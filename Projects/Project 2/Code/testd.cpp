...
int main() {
	...
	double omega_r = 5.0; // 0.01 , 0.50 , 1.00 , 5.00 
	string outfilename;
	rho_min = 0.0;
	rho_max = 2.0;
	outfilename = "NormalWavefuncOmega5.txt";
	/////////////////////////////
	h = (rho_max - rho_min)/n; 
	e = -1.0/(h*h);
	vec d(n+1);
	...
	for (int i=0; i < n-1; i++) { 
		d(i) = 2/(h*h) + omega_r*omega_r*rho(i+1)*rho(i+1) + 1.0/rho(i+1); 
	}
	...
	// Writing all the normalized results to a txt file and plot them // with a python script.
